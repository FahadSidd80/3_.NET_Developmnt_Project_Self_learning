...
begin
	...
	else if (@action='GETRECRUITERPROFILE_ByAdmin') -- new enhancement
	begin
	select ROW_NUMBER()over(order by recruiterid) as SerialNumber, * from tblRecruiter
	join tblCountry on tblRecruiter.country= tblCountry.countryid
	join tblState on tblRecruiter.state= tblState.stateid
	join tblCity on tblRecruiter.city= tblCity.cityid
	where tblRecruiter.status=0
	end
...
alter proc sp_tblAdmin -- new enhancement on 24/08/23
...
@noofvacancies varchar(100)=null,
@comment varchar(100)= null,
@minexperince varchar(100)=null,
@maxexperince varchar(100)=null
...
begin
	...
	--where recruiterid=@recruiterid and tblRecruiter.status=0
	--end
	if (@action='GETRECRUITERPROFILE_ByAdmin') -- new enhancement
	begin
	select ROW_NUMBER()over(order by recruiterid) as SerialNumber, * from tblRecruiter
	join tblCountry on tblRecruiter.country= tblCountry.countryid
	join tblState on tblRecruiter.state= tblState.stateid
	...
	else if (@action= 'GetUserProfileByAdmin') -- enhancement on 30/08/23
	begin
	select ROW_NUMBER()over(order by jid) as SerialNumber, CONVERT(varchar,tblJobseeker.inserted_date,13)  As registerdate, * from tblJobseeker 
	join tblCountry on tblJobseeker.country = tblCountry.countryid
	join tblState on tblJobseeker.state = tblState.stateid
	join tblCity on tblJobseeker.city = tblCity.cityid
	join tblQualification on tblJobseeker.qualification = tblQualification.qualificationid
	join tblStream on tblJobseeker.stream = tblStream.streamid
	join tblJobprofile on tblJobseeker.jobprofile = tblJobprofile.jobprofileid
	where tblJobseeker.status= 0
	end
	else if (@action = 'GetManageJobPostByAdmin')
	begin
	select  ROW_NUMBER()over(order by jobid) as SerialNumber, CONVERT(varchar,tblJobPostRecruiter.inserted_date,13)  As registerdate,  * from tblJobPostRecruiter 
	...
	else if (@action = 'GetFilterJobs_admin')
	begin
	select ROW_NUMBER()over(order by jobid) as SerialNumber,CONVERT(varchar,tblJobPostRecruiter.inserted_date,13)  As registerdate,* from tblJobPostRecruiter 
		join tblJobprofile on  tblJobPostRecruiter.jobprofileid = tblJobprofile.jobprofileid
		join tblRecruiter on tblJobPostRecruiter.recruiterid = tblRecruiter.recruiterid 
		where tblJobPostRecruiter.jobprofileid=@jobtitleid and tblJobPostRecruiter.recruiterid=@companyname
	...
	else if (@action='DELETEJOB_Admin')
	begin
	...
	else if(@action='DELETERECRUITERPROFILE')
	begin
	update tblRecruiter set status=1 where recruiterid=@recruiterid
	end
	else if(@action = 'EditJobPost_ByAdmin')
	begin
	select * from tblJobPostRecruiter where tblJobPostRecruiter.jobid=@jobid and tblJobPostRecruiter.status=0
	end
	else if (@action='UpdateJobPost_ByAdmin')
	begin
	update tblJobPostRecruiter set jobprofileid=@jobtitleid, recruiterid=@recruiterid,
	minexperience=@minexperince,maxexperience=@maxexperince,minsalary=@minsalary,
	maxsalary=@maxsalary,vacancies=@noofvacancies,comment=@comment,inserted_date=GETDATE()
	where tblJobPostRecruiter.status=0 and tblJobPostRecruiter.jobid=@jobid
	end
	else if(@action='EditRecruiterDetailsBy_Admin')
	begin
	select * from tblRecruiter where tblRecruiter.recruiterid=@recruiterid and tblRecruiter.status=0
	end
	--else if(@action='GETRECRUITERDETAILS')
	--begin
	--select * from tblRecruiter where recruiterid=@recruiterid
...
select * from tblRecruiter where tblRecruiter.recruiterid=1 and tblRecruiter.status=0
select top(1) * from tblJobprofile
select top(1)* from tblSkills
select  * from tblJobprofile
--------------------*********** END OF ADMIN  CODE   *************---------------------------------------------------
...
begin
		...
		join tblJobprofile on  tblJobPostRecruiter.jobprofileid = tblJobprofile.jobprofileid
		join tblRecruiter on tblJobPostRecruiter.recruiterid = tblRecruiter.recruiterid 
		where tblRecruiter.recruiterid=@recruiterid and  tblJobPostRecruiter.status=0
		end
...
select  ROW_NUMBER()over(order by jobid) as SerialNumber,* from tblJobPostRecruiter 
		...
		join tblJobprofile on  tblJobPostRecruiter.jobprofileid = tblJobprofile.jobprofileid
		join tblRecruiter on tblJobPostRecruiter.recruiterid = tblRecruiter.recruiterid 
		where tblJobPostRecruiter.jobprofileid=2 and tblJobPostRecruiter.recruiterid=2
		and  tblJobPostRecruiter.minsalary<='78000' and tblJobPostRecruiter.maxsalary<='78000'
		and  tblJobPostRecruiter.status=0
	end

		update tblJobPostRecruiter set status=0
		select * from tblJobPostRecruiter  where status =1 
